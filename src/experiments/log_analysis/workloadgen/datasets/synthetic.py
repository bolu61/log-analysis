import jax
from experiments.log_analysis.workloadgen.datasets.dataset import Dataset

from deepspan.core.hmm import interleaved_random_hmm


def masked_process_dataset(key, size, interleaving, states, length):
    """Synthetic dataset of interleaved sequences
    Uses an interleaved_arcsin_markov_chain to generate the sequences.
    """
    mc = interleaved_random_hmm(interleaving, states)
    ...
    def sequence(key):
        ...
        _, (y, _) = jax.lax.scan(wrapper, state, jax.random.split(sequence_subkey, length))
        return y
